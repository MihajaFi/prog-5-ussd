...
class MVolaUSSDMenu {
    ...
    askChoice(): void {
        rl.question("Choisissez une option : ", (choice) => {
            if (choice === "0") {
                ...
                console.log("Désolé, ce menu n'existe pas. Essaie encore.");
                this.showMenus();
            }
        });
    }
    ...
    private askSecretCode(successCallback: () => void) {
        rl.question("Entrez votre code secret pour confirmer : ", (code) => {
            this.simulateProcessing(() => {
                if (code === this.secretCode) {
                    successCallback();
                } else {
                    console.log("❌ Code secret incorrect. Opération annulée.");
                    this.showBackOption();
                }
            });
        });
